...
// ============================================================
// 4. VEHICLE DATA - ข้อมูลเริ่มต้น (อัปเดตล่าสุด 26 รายการ)
// ============================================================
...
// ============================================================
// 5. LOAD & SAVE DATA FROM FIREBASE
// ============================================================
function loadDataFromFirebase() {
    const loadingDiv = document.getElementById('loadingIndicator');
    if (loadingDiv) loadingDiv.style.display = 'block';
    
    db.ref('vehicleData').once('value')
        .then(snapshot => {
            const data = snapshot.val();
            if (data) {
                vehicleData = Object.values(data);
                vehicleData.sort((a, b) => (a.id || 0) - (b.id || 0));
            } else {
            ...
            applyFilters();
            console.log('✅ โหลดข้อมูลจาก Firebase สำเร็จ', vehicleData.length, 'รายการ');
        })
        .catch(error => {
            ...
            if (loadingDiv) loadingDiv.style.display = 'none';
            applyFilters();
            alert('⚠️ ไม่สามารถเชื่อมต่อ Firebase ได้ ใช้ข้อมูลในเครื่องแทน');
        });
}
...
function saveDataToFirebase() {
    ...
    db.ref('vehicleData').set(dataObject)
        .then(() => {
            console.log('✅ บันทึกข้อมูลลง Firebase สำเร็จ');
        })
        .catch(error => {
            console.error('❌ บันทึกข้อมูลลง Firebase ล้มเหลว:', error);
        });
}
...
function getStatusBadge(status) {
    ...
    if (status === 'warning') return '<span class="badge badge-warning">ใกล้หมดอายุ</span>';
...
// ============================================================
let lineChannelToken = localStorage.getItem('lineChannelToken') || '';
let lineUserId = localStorage.getItem('lineUserId') || '';
let notifyTime = localStorage.getItem('notifyTime') || '08:00';

function openLineModal() {
    document.getElementById('lineTokenInput').value = lineChannelToken;
    document.getElementById('lineUserIdInput').value = lineUserId;
...
function saveLineSettings() {
    lineChannelToken = document.getElementById('lineTokenInput').value.trim();
    lineUserId = document.getElementById('lineUserIdInput').value.trim();
    notifyTime = document.getElementById('notifyTimeInput').value;
    
    localStorage.setItem('lineChannelToken', lineChannelToken);
...
async function testLineConnection() {
    if (!lineChannelToken || !lineUserId) {
        alert('⚠️ กรุณากรอก Channel Access Token และ User ID');
        return;
    }
    ...
    try {
        const response = await fetch('https://api.line.me/v2/bot/message/push', {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        type: 'text',
                        text: '✅ เชื่อมต่อ LINE Messaging API สำเร็จ!'
                    }
                ]
            })
        });
        ...
        if (response.ok) {
            alert('✅ ทดสอบส่งข้อความสำเร็จ!');
            updateLineStatus(true, 'เชื่อมต่อ LINE สำเร็จ');
        } else {
            const error = await response.text();
            console.error('LINE API Error:', error);
            let errorMsg = '❌ ทดสอบไม่สำเร็จ: ';
            if (response.status === 401) errorMsg += 'Token ไม่ถูกต้อง';
            else if (response.status === 404) errorMsg += 'User ID ไม่ถูกต้อง';
            else errorMsg += error;
            alert(errorMsg);
            updateLineStatus(false, 'เชื่อมต่อไม่สำเร็จ');
        }
    } catch (error) {
        console.error('LINE Error:', error);
        let errorMsg = '❌ เกิดข้อผิดพลาด: ';
        if (error.message === 'Failed to fetch') {
            errorMsg += 'ไม่สามารถเชื่อมต่อ LINE API (ใช้ Live Server หรือ HTTPS)';
        } else {
            errorMsg += error.message;
        }
        alert(errorMsg);
        updateLineStatus(false, 'เกิดข้อผิดพลาด');
    }
}

function updateLineStatus(connected, message) {
    ...
    statusDiv.innerHTML = `
        ${connected ? '✅' : '❌'} ${message}
        <button class="btn-test" onclick="openLineModal()">⚙️ ตั้งค่า</button>
    `;
}

async function sendLineNotification() {
    if (!lineChannelToken || !lineUserId) {
        alert('⚠️ กรุณาตั้งค่า LINE Messaging API ก่อน');
        openLineModal();
        return;
    }
    ...
    let message = '📋 แจ้งเตือนเอกสารใกล้หมดอายุ (2 เดือน)\n';
    message += '═'.repeat(20) + '\n\n';
    
    expiring.slice(0, 5).forEach(item => {
        message += `🚗 ทะเบียน: ${item.plate}\n`;
        message += `👤 ผู้ขับขี่: ${item.driver}\n`;
        if (getDateStatus(item.license) === 'warning') {
            message += `📄 ใบขับขี่: ${formatDateDisplay(item.license)} (⚠️ ใกล้หมดอายุ)\n`;
        }
        if (getDateStatus(item.tax) === 'warning') {
            message += `📄 ภาษี: ${formatDateDisplay(item.tax)} (⚠️ ใกล้หมดอายุ)\n`;
        }
        if (getDateStatus(item.prb) === 'warning') {
            message += `📄 พรบ.: ${formatDateDisplay(item.prb)} (⚠️ ใกล้หมดอายุ)\n`;
        }
        message += '\n';
    });

    if (expiring.length > 5) {
        message += `... และอีก ${expiring.length - 5} รายการ\n`;
    }
    
    message += '\n⚠️ กรุณาต่ออายุเอกสารโดยด่วน';
        ...
        const response = await fetch('https://api.line.me/v2/bot/message/push', {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    ...
                    { type: 'text', text: `📊 จำนวนเอกสารใกล้หมดอายุทั้งหมด: ${expiring.length} รายการ` }
                ]
            })
        });

        if (response.ok) {
            alert(`✅ ส่งแจ้งเตือนทาง LINE สำเร็จ! (${expiring.length} รายการ)`);
        } else {
            const error = await response.text();
            console.error('LINE API Error:', error);
            if (response.status === 401) {
                alert('❌ Token ไม่ถูกต้อง กรุณาตั้งค่าใหม่');
                openLineModal();
            } else {
                alert('❌ ส่งข้อความไม่สำเร็จ: ' + error);
            }
        }
    } catch (error) {
        console.error('LINE Error:', error);
...
function checkLineConnection() {
    if (lineChannelToken && lineUserId) {
        updateLineStatus(true, 'เชื่อมต่อ LINE สำเร็จ (พร้อมใช้งาน)');
    } else {
        const statusDiv = document.getElementById('lineStatus');
        if (statusDiv) {
            statusDiv.style.display = 'flex';
            ...
            statusDiv.innerHTML = `
                ❌ ยังไม่ได้ตั้งค่า LINE Messaging API
                <button class="btn-test" onclick="openLineModal()">⚙️ ตั้งค่า</button>
            `;
        }
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(checkLineConnection, 2000);
    ...
    console.log('🚀 ระบบพร้อมใช้งานแล้ว (เชื่อมต่อ Firebase - Cardox)');
    ...
    console.log('💬 ใช้ LINE Messaging API สำหรับแจ้งเตือน');
